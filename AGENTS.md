...
- 운영 로그 보존 기간은 최대 6개월을 기준으로 한다.
- 프로젝트별 `배포방법.md`에 빌드·게시·복구 절차와 자동 버전 정책을 기록한다.
- 권장 최상위 폴더 구조는 신규 프로젝트에만 적용하며 기존 구조는 변경하지 않는다.
- 적용 전 `-WhatIf`, 적용 후 `Test-DeveloperKit.ps1` 점검을 수행한다.
